...
public class AccountServiceImpl implements AccountService {
	...
	public void addAccount(Account account) {
		...
		if (accounts.containsKey(username)) {
			throw new RuntimeException("Error of synchronization between account and accounts manager");
		}
	...
	private void createUser(Account user) {
		...
		userDetailsManager.createUser(User.withUsername(user.getUsername()).password(user.getPassword())
				.roles(user.getRoles()).accountExpired(LocalDateTime.now().compareTo(user.getExpDate()) <= 0).build());
	...
	private void expirationValidation() {
		...
		accounts.values().stream().filter(this::isExpired).forEach(
				a -> userDetailsManager.updateUser(User.withUsername(a.getUsername()).accountExpired(true).build()));
	}
